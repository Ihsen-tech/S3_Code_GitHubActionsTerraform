...
resource "null_resource" "link_monitoring" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      az login --service-principal -u $con_client_id -p $con_client_secret --tenant $con_tenant_id || exit 3

      # Set Application Insights configurations on the web app
      az webapp config appsettings set --name $web_app_name --resource-group $rg_name --settings \
        APPINSIGHTS_INSTRUMENTATIONKEY=$inst_key \
        ...
        XDT_MicrosoftApplicationInsights_BaseExtensions=recommended \
        XDT_MicrosoftApplicationInsights_PreemptSdk=disabled || exit 3
    EOT
    environment = {
